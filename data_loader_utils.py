...
def batch_load_hyperspectral_data(data_dir: str, 
                                 file_pattern: str = "*.hdr",
                                 label_from_filename: bool = True) -> Tuple[np.ndarray, List[str]]:
    """
    Batch load multiple hyperspectral files
    
    Args:
        data_dir: Directory containing hyperspectral files
        file_pattern: Pattern to match files (default: "*.hdr")
        label_from_filename: Whether to extract labels from filenames
        
    Returns:
        Tuple of (spectra_array, labels_list)
    """
    from main_implementation import HyperspectralDataLoader
    ...
    loader = HyperspectralDataLoader(str(data_dir))
    spectra = []
    labels = []
    
    for file_path in files:
        try:
            img_array = loader.load_envi_image(str(file_path))
            
            if img_array is not None:
                # Extract mean spectrum or use other extraction method
                if len(img_array.shape) == 3:
                    # Hyperspectral image - extract mean spectrum
                ...
                # Extract label from filename if requested
                if label_from_filename:
                    filename = file_path.stem.lower()
                    if 'healthy' in filename or 'normal' in filename:
                        labels.append('healthy')
                    elif 'biotic' in filename or 'disease' in filename:
                        labels.append('biotic')
                    elif 'abiotic' in filename or 'stress' in filename:
    ...
    if not spectra:
        logger.error("No valid spectra were loaded")
        return np.array([]), []
    
    spectra_array = np.array(spectra)
    logger.info(f"Loaded {len(spectra)} spectra with shape {spectra_array.shape}")
    
    return spectra_array, labels